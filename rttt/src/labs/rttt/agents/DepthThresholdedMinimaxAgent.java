...
public class DepthThresholdedMinimaxAgent
    ...
    public Node minimax(Node node) {
        ...
        // if beyond max depth (also base case)
        if (node.getDepth() >= this.maxDepth) {
            double value = Heuristics.calculateHeuristicValue(node);
        ...
        // max player
        if (node.getCurrentPlayerType().equals(node.getMyPlayerType())) {
            bestVal = Double.NEGATIVE_INFINITY;
            for (Node child : children) {
                Node value = minimax(child);
                if (value.getUtilityValue() > bestVal) {
                    bestChild = value;
                    bestVal = value.getUtilityValue();
                }
            }
        } else { // minplayer
